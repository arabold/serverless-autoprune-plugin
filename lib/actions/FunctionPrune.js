			...
			const pruneFunc = (func) => {
				...
				return BbPromise.each(lambdaNames, (lambdaName) => {
					return BbPromise.join(
						_this.aws.request('Lambda', 'listAliases', { FunctionName: lambdaName }, _this.evt.options.stage, _this.evt.options.region),
						_this.aws.request('Lambda', 'listVersionsByFunction', { FunctionName: lambdaName }, _this.evt.options.stage, _this.evt.options.region)
						...
						return BbPromise.map(versions.Versions, (v) => {
							if (keepVersions.indexOf( v.Version ) < 0) {
								...
								return _this.aws.request('Lambda', 'deleteFunction', {
