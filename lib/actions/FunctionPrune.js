...
module.exports = function(S) {
	...
	class FunctionPrune extends S.classes.Plugin {
		...
		registerActions() {
			...
			S.addAction(this.FunctionPrune.bind(this), {
		...
		_slowdownRequests(f) {
			return new BbPromise( function( resolve, reject ){
				let doCall = function() {
					...
					.then(resolve)
					.catch(function(err) {
						if( err.cause && err.cause.code === 'TooManyRequestsException' ) {
		...
		_pruneByRegion(region) {
			let _this = this;
			let Lambda = BbPromise.promisifyAll(new AWS.Lambda({ region: region }), { suffix: 'Promised' });
			...
			const pruneFunc = (func) => {
				...
				return BbPromise.each(lambdaNames, (lambdaName) => {
					return BbPromise.join(
						_this._slowdownRequests(() => {
							return Lambda.listAliasesPromised({ FunctionName: lambdaName });
						}),
						_this._slowdownRequests(() => {
							return Lambda.listVersionsByFunctionPromised({ FunctionName: lambdaName });
						})
						...
						return BbPromise.map(versions.Versions, (v) => {
							if (keepVersions.indexOf( v.Version ) < 0) {
								...
								return _this._slowdownRequests(function() {
									return Lambda.deleteFunctionPromised({
